...
function sln_init($) {
    if ($("#salon-step-services").length || $("#salon-step-secondary").length) {
        ...
        if(request_args.find(val => val.startsWith('save_selected')) || request_args.find(val => val.startsWith('submit_services') || val.startsWith('?submit_services'))){
            $(document).scrollTop($('#sln-salon').offset().top);
            $('#salon-step-services .sln-box--fixed_height').scrollTop($('#salon-step-services input[type="checkbox"][checked="checked"]').offset().top - $('#salon-step-services input[type="checkbox"]').first().offset().top - 100);
        }
...
+(function($) {
    function sln_tabsFrontEnd() {
        $(".sln-content__tabs__nav__item--- a").click(function(e){
            e.preventDefault();
            console.log("poll");
            $(this).tab('show');
        });
        $(".sln-content__tabs__nav__item a").each(function () {
